...
Notice that two equal substrings always share a common fingerprint, but the inverse is not true. It has been %proven
...
{\color{red}{Algorithm~\ref{alg:2} mainly consists of two steps, where the first step for checking $sa^*$ and $lcp^*$ can be done within sorting complexity and the second step for checking $sa$ and $lcp$ can also be done in sorting complexity for using external-memory sorters and priority queues. In our current program for this algorithm, the peak disk use is reached in step two, specifically, when computing the BWT from $sa$ and $x$ for use in the inducing process. }}
