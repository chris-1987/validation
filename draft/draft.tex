...
Given an input string $x[0, n)$ drawn from an alphabet $\{\Sigma \cup \$\}$, we suppose $t

suppose $\$$ to be smaller than any other characters and only appears in $x[n - 1]$. We denote by ${\sf sub}(i, j)$ a substring of $x$ starting from $x[i]$ to $x[j]$. Paricularly, if $j = n - 1$, we say ${\sf sub}(i, j)$ is a suffix of $x$ and represent it by ${\sf suf}(i)$ for the sake of description.
The suffix array of $x$,
