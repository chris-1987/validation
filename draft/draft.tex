...
As will be seen from our experimental study in Section~\ref{sec:experiment}, Algorithm~\ref{alg:1} is quite space consuming, the peak disk use is 40 bytes per input character. This motivates us to design a lightweight checker using less space. In this section, we propose a method for checking the suffix and LCP arrays based on the induced-sorting principle. In comparison with Algorithm~\ref{alg:1}, the algorithm designed by this method only consumes about half space on real-world data sets.

\subsection{Preliminaries} \label{sec:algorithm2:preliminaries}

The induced sorting principle was previously employed to design algorithms on building suffix and LCP arrays in internal and external memory. The basic idea is to induce lexical order and LCP-values of all the suffixes from those of a subset.
